...
| `data-gum.js` | All 43 GUM tables (1,580 rows) + `GUM_FOR_FIELDS`, `GUM_PLOT_SEED`, `GUM_GRAND`, `INSPIRE_WORDS`, `GUM_ERRATA` |
...
| `ui.js` | `modal/toast/confirmModal/promptModal`, `explain()`, `actionBar()`, `defRow()`, and the `registerInspire`/`inspireBlock` seam that lets a text field roll without ui.js knowing GUM exists |
...
| `forge.js` | GUM: the generator tab, the plot seed, the grand oracle, and the inspiration block — three rolled words mounted inside every text field in the app |
...
| Any blank can ask GUM for three words | Lookup | `GUM_FOR_FIELDS`, `INSPIRE_WORDS` | `forge.inspireTables`, `roller.rollGum` | A collapsed block in every text field | `every text dialog offers an inspiration roll` |
...
- [x] **Phase 8 — Inspiration everywhere**: every text field in the app can roll three
      context-matched GUM words, appended not imposed, behind one collapsed line.
