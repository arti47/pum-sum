...
| `tests/` | Harnesses (`harness`, `smoke`, `interaction`, `audit-modals`, `audit-deep`, `deadcode`), probes (`probe-layout`, `probe-flow`), seed fixtures, and `tests/tools/` — the retained extraction scripts plus the two guide generators (`gen-tutorial`, `gen-pdf`) |
